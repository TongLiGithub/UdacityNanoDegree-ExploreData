...
```{r echo=FALSE, message = FALSE, warning = FALSE}
qplot(x=MonthlyLoanPayment, data=data_var)
```

```{r echo=FALSE, message = FALSE, warning = FALSE}
qplot(x=Investors, data=data_var)
```
...
```{r echo=FALSE, message = FALSE, warning = FALSE}
qplot(x=CreditScoreRangeLower, data=data_var)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=Investors, y=LoanOriginalAmount), data=newdata) +
    geom_point(alpha=1/5) +
    geom_smooth(method="lm", color='red')
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=LP_CustomerPayments, y=LoanOriginalAmount), 
       data=newdata) + 
    geom_point(alpha=1/5) +
    geom_smooth(method="lm", color='red')
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=LP_ServiceFees, y=LoanOriginalAmount), 
       data=newdata) + 
    geom_point(alpha=1/5) +
    geom_smooth(method="lm", color='red')
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=LP_CustomerPayments, y=LP_ServiceFees), data=newdata) +
    geom_point(alpha=1/5) +
    geom_smooth(method='lm', color='red')
```
...
ggplot(aes(x=Investors, y=LoanOriginalAmount), data=newdata) +
    geom_point(alpha=1/5) +
    facet_wrap(~LoanOriginalQuarter) +
    geom_smooth(method='lm', color='red') 
```
...
m8<-update(m7, . ~ . + EmploymentStatus2)
m9<-update(m8, . ~ . + MonthlyLoanPayment:Term)
m10<-update(m9, . ~ . + IncomeRange:Investors)
m11<-update(m10, . ~ . + Term:Investors)

anova(m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11)
mtable(m1,m2,m3,m4, sdigits=3)
mtable(m4,m5,m6,m7, sdigits=3)
mtable(m7,m8,m9, sdigits=3)
mtable(m9,m10,m11, sdigits=3)
```
